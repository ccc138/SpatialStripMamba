...
from slamamba import Mamba, MambaConfig 
...
class MyModel(torch.nn.Module):
    def __init__(self):
        ...
        self.conv01 = BasicConv(in_channels=icc1, out_channels=ic, kernel_size=1, stride=1, padding=0) 
        self.conv02 = BasicConv(in_channels=icc2, out_channels=ic, kernel_size=1, stride=1, padding=0) 
        self.conv1=BasicConv(in_channels=ic*2, out_channels=ic, kernel_size=1, stride=1, padding=0) 

        self.inc= DoubleConv(ic,oc)
        self.down1=PNC(oc,oc*2) 
        self.down2=PNC(oc*2,oc*4) 
        
        #self.sf1=StateFusion(oc*2)
        self.tb1=TransBlock(oc*2,fs-1,num_encoder_layers=3) 
        
        self.up2= UpWOconv(oc*2,oc) #UpWOconv(oc*2,oc)
        self.up1= UpWOconv(oc*4,oc*2)
        self.tbU2=TransBlock(oc,ws,num_encoder_layers=3) 
        #self.mbas2=MBAS(oc)
        ...
        self.tb02=TransBlock(ic,ws,num_encoder_layers=1) 
        self.ca=StandardCA(ic)
        self.lmd = nn.Parameter(torch.tensor(0.))

        self.mamba0b = MambaLayers(d_model=ic,n_layers=2)
        self.tbb=TransBlock(ic,ws,num_encoder_layers=1)  
        self.tb02b=TransBlock(ic,ws,num_encoder_layers=1) 
        self.cab=StandardCA(ic)
        self.lmdb = nn.Parameter(torch.tensor(0.))
    ...
    def forward(self, x):
        ...
        x1=self.tb02(x1).permute(0, 1, 3, 2)
        x=self.tb(x)
        ...
        xb=self.mamba0b(xb)
        x1=xb.permute(0, 1, 3, 2)
        x1=self.tb02b(x1).permute(0, 1, 3, 2)
        xb=self.tbb(xb)

        lmdb=torch.sigmoid(self.lmdb)
        xb=lmdb*x1+(1-lmdb)*xb
        xb=self.cab(xb)

        x=self.conv1(torch.cat((x,xb),1))
        del xb
