...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.local_norm = RMSNorm(config.d_model, config.rms_norm_eps, config.mup)
        # ==============================================
    ...
    def forward(self, x):
        ...
        # y : (B, L, D)
 # ===== Apply Local Enhanced Module (LEM) =====
        B, L, C = x.shape; H = W = int(L**0.5)
        x_2d = x.transpose(1, 2).view(B, C, H, W)  # [B, C, H, W]
        local_feat = self.local_conv(x_2d)
        local_feat = local_feat.view(B, C, L).transpose(1, 2)  # [B, L, C]
        x = x + self.local_norm(local_feat)  # residual connection
        # ==========================================
